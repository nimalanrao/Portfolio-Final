...
export function LazyImage({
	...
	return (
		<AspectRatio
			...
			ratio={ratio}
			className={cn(
				'relative size-full overflow-hidden rounded-lg border border-white/5',
				AspectRatioClassName,
			)}
		>
			{/* Skeleton / fallback */}
			<div
				className={cn(
					'bg-white/5 absolute inset-0 animate-pulse rounded-lg transition-opacity will-change-[opacity]',
					{ 'opacity-0': !isLoading },
				)}
			/>
